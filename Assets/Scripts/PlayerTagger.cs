...
public class PlayerTagger : NetworkBehaviour
{
    [SerializeField] private float tagRange = 2.5f;
    [SerializeField] private KeyCode tagKey = KeyCode.Q;
    [SerializeField] private LayerMask playerLayer;
    [SerializeField] private UI winScreen;
    
    public static Action onTagWin { get; internal set; }
    public float gameState;

    public static Action<string> onTagVictory;
    public static Action<string> onGameOutcome;

    void FixedUpdate()
    {
        PlayerTagger nearbyPlayer = FindNearbyPlayer();
    }
    
    PlayerTagger FindNearbyPlayer()
    {
        // Sphere check for nearby players
        Collider[] colliders = Physics.OverlapSphere(transform.position, tagRange, playerLayer);
        
        foreach (Collider col in colliders)
        {
            PlayerTagger player = col.GetComponent<PlayerTagger>();
            if (player != null && player != this && !player.isLocalPlayer)
            {
                return player;
            }
        }
    ...
    {
        PlayerTagger nearbyPlayer = FindNearbyPlayer();
        if (Input.GetKeyDown(tagKey) && nearbyPlayer != null)
        {
            onGameOutcome.Invoke(gameState.victory); 
        } 
    }
}
*/